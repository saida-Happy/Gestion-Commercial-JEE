...
public class CommandesDAO extends BaseHibernateDAO {
	...
	// property constants
	public static final String NOM_CLT = "nomClt";
	...
	public Commandes findById(java.lang.Integer id) {
		...
		try {
			Commandes instance = (Commandes) getSession().get("beans.Commandes",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Commandes instance) {
		...
		try {
			List results = getSession().createCriteria("beans.Commandes").add(
	...
	public List findByNomClt(Object nomClt) {
		return findByProperty(NOM_CLT, nomClt);
	}
